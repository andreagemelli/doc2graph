...
class SetModel:
    ...
    def get_model(
        self, nodes: int, edges: int, chunks: list, verbatim: bool = True
    ) -> nn.Module:
        ...
        if self.name == "GCN":
            ...
            m = EdgeClassifier(
                ...
                self.device,
                self.cfg_model.doProject,
            )

        elif self.name == "E2E":
            edge_pred_features = int(
                (math.log2(get_config("preprocessing").FEATURES.num_polar_bins) + nodes)
                * 2
            )
            m = E2E(
                nodes,
...
class EdgeClassifier(nn.Module):
    def __init__(
        ...
        for layer_idx in range(m_layers):
            self.message_passing.append(GcnSAGELayer(m_hidden, m_hidden, F.relu, 0.0))
    ...
    def forward(self, g, h):
        ...
        for layer_idx in range(self.m_layers):
            h = self.message_passing[layer_idx](g, h)
...
class E2E(nn.Module):
    def __init__(
        ...
        self.message_passing = GcnSAGELayer(m_hidden, m_hidden, F.relu, 0.0)

        # Define edge predictor layer
        self.edge_pred = MLPPredictor_E2E(
            m_hidden, hidden_dim, edge_classes, dropout, edge_pred_features
        )
...
class GcnSAGELayer(nn.Module):
    ...
    def forward(self, g, h):
        ...
        if not self.use_pp:
            ...
            norm = g.ndata["norm"]
            g.ndata["h"] = h
            g.update_all(fn.u_mul_e("h", "weights", "m"), fn.sum(msg="m", out="h"))
            ah = g.ndata.pop("h")
...
class MLPPredictor_E2E(nn.Module):
    ...
    def apply_edges(self, edges):
        ...
        x = F.relu(
            self.norm(self.W1(torch.cat((h_u, cls_u, polar, h_v, cls_v), dim=1)))
        )
        score = self.drop(self.W2(x))
