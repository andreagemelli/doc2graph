...
from ..data.dataloader import Document2Graph
from ..paths import (
    FUNSD_TRAIN,
    FUNSD_TEST,
    TRAIN_SAMPLES,
...
def e2e(args):
    ...
    for m in models:
        model.load_state_dict(torch.load(CHECKPOINTS / m, map_location=device))
        model.eval()
        with torch.no_grad():
            ...
            _, classes_f1 = get_binary_accuracy_and_f1(
                preds, test_graph.edata["label"], per_class=True
            )
        ...
        print("\n### RESULTS {} ###".format(m))
        print(
            "F1 Edges: None {:.4f} - Pairs {:.4f}".format(classes_f1[0], classes_f1[1])
        )
        print("F1 Nodes: Macro {:.4f} - Micro {:.4f}".format(macro, micro))

    print(f"\n -> Loading best model {best_model}")
    model.load_state_dict(torch.load(CHECKPOINTS / best_model, map_location=device))
    model.eval()
    with torch.no_grad():
        n, e = model(test_graph, test_graph.ndata["feat"].to(device))
        auc = compute_auc_mc(e.to(device), test_graph.edata["label"].to(device))
    ...
    return {
        "LINKS [MAX, MEAN, STD]": [classes_f1[1], mean(edges_f1), np.std(edges_f1)],
        "NODES [MAX, MEAN, STD]": [micro, mean(nodes_micro), np.std(nodes_micro)],
    }
...
def entity_linking(args):
    ...
    if not args.test:
        ...
        for cvs in cv_indices:
            ...
            models.append(train_name + ".pt")
            stopper = EarlyStopping(
                model, name=train_name, metric=cfg_train.stopper_metric, patience=1000
            )
            # writer = SummaryWriter(log_dir=RUNS)
            # convert_imgs = transforms.ToTensor()
            ...
            for epoch in range(cfg_train.epochs):
                ...
                loss = compute_crossentropy_loss(
                    scores.to(device), tg.edata["label"].to(device)
                )
                ...
                with torch.no_grad():
                    ...
                    val_loss = compute_crossentropy_loss(
                        val_scores.to(device), vg.edata["label"].to(device)
                    )
                    val_auc = compute_auc_mc(
                        val_scores.to(device), vg.edata["label"].to(device)
                    )
                ...
                print(
                    "Epoch {:05d} | TrainLoss {:.4f} | TrainAUC-PR {:.4f} | ValLoss {:.4f} | ValAUC-PR {:.4f} |".format(
                        epoch, loss.item(), auc, val_loss.item(), val_auc
                    )
                )
    ...
    for m in models:
        ...
        with torch.no_grad():
            ...
            auc = compute_auc_mc(
                scores.to(device), test_graph.edata["label"].to(device)
            )
        ...
        print(
            "F1 Classes: None {:.4f} - Pairs {:.4f}".format(
                classes_f1[0], classes_f1[1]
            )
        )
