...
def polar(rect_src: list, rect_dst: list) -> Tuple[int, int]:
    ...
    vp_intersect = (
        rect_src[0] <= rect_dst[2] and rect_dst[0] <= rect_src[2]
    )  # True if two rects "see" each other vertically, above or under
    hp_intersect = (
        rect_src[1] <= rect_dst[3] and rect_dst[1] <= rect_src[3]
...
def to_bin(dist: int, angle: int, b=8) -> torch.Tensor:
    ...
    return torch.cat(
        [
            torch.tensor(new_dist, dtype=torch.float32),
            torch.tensor(new_angle, dtype=torch.float32),
        ],
        dim=1,
    )
