...
def e2e(args):
    ...
    seed(cfg_train.seed)
    sm = Doc2GraphModel(name=args.model, device=DEVICE)

    if not args.test:
        ################* STEP 0: LOAD DATA ################
        data = Doc2GraphLoader(name='PAU TRAIN', src_path=PAU_TRAIN, output_dir=TRAIN_SAMPLES)
        data.get_info()
    ...
    #? test
    test_data = Doc2GraphLoader(name='PAU TEST', src_path=PAU_TEST, output_dir=TEST_SAMPLES)
    test_data.get_info()
